...
public class SampleController {
    ...
    @GetMapping(value = { "front-entry" })
    public String frontEntry() {
        try {
            LOGGER.info("Front service receive request");
            LOGGER.info(String.format("Sending request to %s", this.middleServiceUrl));

            this.sleepForRandomPeriod();
            this.randomlyThrowException();

            ServiceResponseDTO responseObject = this.restTemplate.getForObject(this.middleServiceUrl,
                ServiceResponseDTO.class);

            LOGGER.info(String.format("Front service receive response: %s",
